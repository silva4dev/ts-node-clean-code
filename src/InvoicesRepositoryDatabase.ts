...
export default class InvoicesRepositoryDatabase implements InvoicesRepository {
	...
	async getInvoices(): Promise<Invoice[]> {
		...
		const invoicesData = await connection.query("select * from invoice", []);
		const invoices = [];
		for (const invoiceData of invoicesData) {
			const invoice = new Invoice(invoiceData.invoice_id, invoiceData.due_date);
			const transactionsData = await connection.query("select * from transaction t where invoice_id = $1", [invoice.invoiceId]);
